...
void WelcomeNewClient(RoomInfo* ri, int id)
{
	...
	mw.special = 0;
#if 0
	if (GetWeekDay() == 7) mw.special = 1; // Lunes
	if (GetWeekDay() == 6 || GetWeekDay() == 1 || GetWeekDay() == 5) mw.special = 2; // Martes
	if (GetWeekDay() == 6 || GetWeekDay() == 4 || GetWeekDay() == 2) mw.special = 3; // Miercoles
#endif
