...
void DECOMP_UI_DrawRaceClock(u_short paramX, u_short paramY, u_int flags, struct Driver* driver)
{
	...
	if ((driver->driverID == 0) && (driver->lapIndex > 0))
	{
		...
		DECOMP_DecalFont_DrawLine("MEJOR: ", posX, textPosY + 0x18, FONT_SMALL, TROPY_LIGHT_BLUE);
		DECOMP_DecalFont_DrawLine(displayTime, posX + data.font_charPixWidth[FONT_SMALL] * 6, textPosY + 0x18, FONT_SMALL, PENTA_WHITE);

		ElapsedTimeToTotalTime(&tt, driver->currLapTime);
		tt.minutes = min(tt.minutes, 9);
		sprintf(displayTime, "%d:%02d.%03d", tt.minutes, tt.seconds, tt.miliseconds);
		DECOMP_DecalFont_DrawLine("ULTIMA: ", posX, textPosY + 0x18 + 8, FONT_SMALL, FAKE_CRASH_GRAY);
		DECOMP_DecalFont_DrawLine(displayTime, posX + data.font_charPixWidth[FONT_SMALL] * 7, textPosY + 0x18 + 8, FONT_SMALL, PENTA_WHITE);
	}
	#else
	if
	(
