...
{
    const char s_switchCam[] = "Cambia de camara con L1 o R1";
...
void EndOfRace_Icons()
{
    ...
    for (int i = 0; i < octr->NumDrivers; i++)
    {
        ...
        if (tt.hours > 0)
        {
            sprintf(s_time, "%d:%02d:%02d:%02d", tt.hours, tt.minutes, tt.minutes, tt.seconds);
        }
        else
        {
            sprintf(s_time, "%02d:%02d.%03d", tt.minutes, tt.seconds, tt.miliseconds);
        }
